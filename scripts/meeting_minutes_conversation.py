...
def extract_problem_context(sentence: str) -> dict[str, str] | None:
    ...
    patterns = [
        ...
        (r"^(?:the\s+)?(?P<subject>.+?)\s+is\s+unclear\b", "Refine"),
        (r"^(?:the\s+)?(?P<subject>.+?)\s+is\s+confusing\b", "Clarify"),
        (r"^(?:the\s+)?(?P<subject>.+?)\s+feels\s+confusing\b", "Clarify"),
        (r"^(?:the\s+)?(?P<subject>.+?)\s+feels\s+rushed\b", "Improve"),
        (r"^(?:the\s+)?(?P<subject>.+?)\s+is\s+too\s+long\b", "Tighten"),
        (r"^(?:the\s+)?(?P<subject>.+?)\s+is\s+too\s+short\b", "Expand"),
        (r"^(?:the\s+)?(?P<subject>.+?)\s+needs\s+work\b", "Improve"),
        (r"^(?:the\s+)?(?P<subject>.+?)\s+isn['’]?t\s+landing\b", "Improve"),
        (r"^(?:the\s+)?(?P<subject>.+?)\s+is\s+not\s+landing\b", "Improve"),
