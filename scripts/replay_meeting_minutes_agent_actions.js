...
(async () => {
  ...
  for (const j of kept.journeys) {
    ...
    const draft = { ...payload, draftId: `replay-${j.draftId}`, revision: 1, actions: [], pendingProposal: null };
    const started = Date.now();
    let result;
    try { result = await api.generateHybridMeetingAgentStage(draft, 'actions', { onProgress: () => {}, onCheckpoint: () => {}, onPreview: () => {} }); }
