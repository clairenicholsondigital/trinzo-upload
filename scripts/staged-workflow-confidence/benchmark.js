...
function renderDashboard(report, previous = null) {
  ...
  const comparison = previous ? `<p>Previous revision: <code>${escapeHtml(previous.servingRevision || 'unknown')}</code>; action recall ${previous.summary.actionRecallMean} → ${report.summary.actionRecallMean}; precision ${previous.summary.actionPrecisionMean} → ${report.summary.actionPrecisionMean}.</p>` : '<p>No earlier compatible report was supplied.</p>';
