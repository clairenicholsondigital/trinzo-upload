...
function normaliseMinutes(input = {}) {
  ...
  return {
    ...
    decisions,
    openQuestions,
    supportingDetails: (Array.isArray(input.supportingDetails) ? input.supportingDetails : []).map((item) => ({
      topic: clean(item?.topic, 'Discussion').slice(0, 1000), text: clean(item?.text).slice(0, 5000)
...
function renderStagedMinutesPdfHtml(input = {}) {
  ...
  const { details, executiveSummary, meetingObjectives, discussion, decisions, openQuestions, actions, evidenceAppendix, reviewFlags, supportingDetails } = minutes;
  return `<!doctype html>
