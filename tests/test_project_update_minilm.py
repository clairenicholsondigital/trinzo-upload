...
class ProjectUpdateMiniLMWorkflowTest(unittest.TestCase):
    ...
    def test_project_update_page_enables_editable_report_ui(self):
        ...
        settings_stage = (REPO_DIR / "public" / "project-stage-settings.js").read_text(encoding="utf-8")
        setup_stage = (REPO_DIR / "public" / "project-stage-setup.js").read_text(encoding="utf-8")
        memory_stage = (REPO_DIR / "public" / "project-stage-memory.js").read_text(encoding="utf-8")
        ...
        self.assertIn("/static/project-stage-memory.js", page)
        self.assertIn("/static/project-stage-settings.js", page)
        self.assertIn("stage-tabs", page)
        self.assertIn("Project Updates", page)
        ...
        self.assertIn("Capture this update", process_stage)
        self.assertIn("Use the agenda below, then paste the meeting notes or transcript", process_stage)
        self.assertIn("data-process-panel=\"meeting\" aria-pressed=\"true\">Process meeting</button>", process_stage)
        self.assertIn("data-process-panel=\"agenda\" aria-pressed=\"false\">Suggested agenda</button>", process_stage)
        self.assertIn("data-process-tab-panel=\"meeting\"", process_stage)
        self.assertIn("data-process-tab-panel=\"agenda\" hidden", process_stage)
        self.assertIn("wireProcessSubtabs(container)", process_stage)
        self.assertIn("panel.hidden = panel.getAttribute('data-process-tab-panel') !== target", process_stage)
        self.assertNotIn("Use this as the scaffold for a real update meeting", process_stage)
        self.assertNotIn("meeting-scaffold-grid", process_stage)
        self.assertNotIn("Before the meeting", process_stage)
        ...
        self.assertIn("For each item, confirm what changed, what is blocked", process_stage)
        self.assertIn("Review monitored risks", process_stage)
        self.assertIn("Confirm whether each risk has reduced, escalated", process_stage)
        self.assertIn("Mitigation:", process_stage)
        self.assertIn("Forecast ${PW.escapeHtml(PW.displayDate(item.forecastFinishDate))}", process_stage)
        self.assertNotIn("Ask: what changed, what is blocked, and does the forecast still hold?", process_stage)
        self.assertNotIn("Ask: has this risk changed, reduced, or escalated?", process_stage)
        self.assertNotIn("Check: ${PW.escapeHtml", process_stage)
        ...
        self.assertIn("function openProject(projectId, stageKey)", workspace_shell)
        self.assertIn("openProject(card.getAttribute('data-open-project'))", workspace_shell)
        self.assertIn("project-nav", workspace_shell)
        self.assertIn("data-project-nav=\"overview\"", workspace_shell)
        self.assertIn("data-project-nav=\"setup\"", workspace_shell)
        self.assertIn("data-project-nav=\"memory\"", workspace_shell)
        self.assertIn("data-project-nav=\"process\"", workspace_shell)
        self.assertIn("data-project-nav=\"reports\"", workspace_shell)
        self.assertIn("data-project-nav=\"ask\"", workspace_shell)
        self.assertIn("data-project-nav=\"settings\"", workspace_shell)
        self.assertNotIn("data-project-nav=\"switch\"", workspace_shell)
        self.assertIn("workspaceStageUrl('setup')", workspace_shell)
        self.assertIn("workspaceStageUrl('memory')", workspace_shell)
        self.assertIn("workspaceStageUrl('process')", workspace_shell)
        self.assertIn("workspaceStageUrl('ask')", workspace_shell)
        self.assertIn("goToStage('setup', true)", workspace_shell)
        self.assertIn("goToStage('memory', true)", workspace_shell)
        self.assertIn("goToStage('process', true)", workspace_shell)
        self.assertIn("goToStage('ask', true)", workspace_shell)
        self.assertNotIn("#ask-this-project", workspace_shell)
        self.assertIn("projectChooserUrl", workspace_shell)
        self.assertIn('id="switchProjectLink"', workspace_shell)
        self.assertIn('class="project-switch-link"', workspace_shell)
        self.assertIn('target="_blank"', workspace_shell)
        self.assertIn('rel="noopener"', workspace_shell)
        self.assertNotIn("switchProjectBtn", workspace_shell)
        self.assertIn("project-summary", workspace_shell)
        self.assertIn("Start with the project overview", workspace_shell)
        ...
        self.assertIn(">Setup</a>", workspace_shell)
        self.assertIn(">Memory</a>", workspace_shell)
        self.assertIn('project-nav-icon" aria-hidden="true">+</span><span>Process meeting</span>', workspace_shell)
        self.assertIn("project-nav-link project-nav-primary", workspace_shell)
        self.assertIn('project-nav-icon" aria-hidden="true">&#128172;</span><span>Ask</span>', workspace_shell)
        self.assertIn(">Reports</a>", workspace_shell)
        self.assertLess(workspace_shell.index(">Process meeting</span></a>"), workspace_shell.index(">Overview</a>"))
        self.assertLess(workspace_shell.index(">Overview</a>"), workspace_shell.index(">Setup</a>"))
        self.assertLess(workspace_shell.index(">Setup</a>"), workspace_shell.index(">Memory</a>"))
        self.assertLess(workspace_shell.index(">Memory</a>"), workspace_shell.index(">Reports</a>"))
        self.assertNotIn('class="project-nav-link icon-only"', workspace_shell)
        self.assertNotIn('aria-label="Settings"', workspace_shell)
        self.assertNotIn("&#9881;", workspace_shell)
        self.assertIn('data-project-nav="settings">Settings</a>', workspace_shell)
        self.assertNotIn("Open project profile", workspace_shell)
        self.assertNotIn("profileBtn.hidden = stageKey === 'insights'", workspace_shell)
        self.assertIn('id="openProjectSettingsBtn"', workspace_shell)
        self.assertIn('id="openProjectProcessBtn"', workspace_shell)
        self.assertIn("openProjectProcessBtn", workspace_shell)
        self.assertIn("openProjectSettingsBtn", workspace_shell)
        self.assertIn("goToStage('settings', true)", workspace_shell)
        self.assertNotIn("Change project", workspace_shell)
        self.assertNotIn("Edit project profile", process_stage)
        self.assertNotIn("Edit details", workspace_shell)
        self.assertNotIn("editProjectBtn", workspace_shell)
        self.assertNotIn("editProjectPanel", workspace_shell)
        self.assertNotIn("exportProjectOverviewBtn", workspace_shell)
        self.assertIn("Query this project", workspace_shell)
        self.assertIn("Settings", workspace_shell)
        self.assertIn("Project settings", settings_stage)
        ...
        self.assertIn("at-glance-strip", page)
        self.assertIn("glance-metrics", page)
        self.assertIn("profile-editor-table", page)
        self.assertIn("profile-title-field", page)
        self.assertIn("monitor-checkbox", page)
        self.assertIn("profile-monitor-column", page)
        self.assertIn("monitor-indicator", page)
        self.assertIn(".project-bar .identity .name { font-size:2.15rem", page)
        self.assertIn(".project-bar .project-summary", page)
        self.assertIn(".ask-panel-standalone .ask-layout", page)
        self.assertIn("grid-template-columns:1fr", page)
        self.assertNotIn(".insights-header-panel h1", page)
        self.assertIn("safeguard-banner.quiet", page)
        self.assertIn("Process meeting", workspace_shell)
        self.assertIn("Overview", workspace_shell)
        self.assertIn("Reports", workspace_shell)
        self.assertIn("Setup", workspace_shell)
        self.assertIn("Memory", workspace_shell)
        self.assertIn("Settings", workspace_shell)
        self.assertIn("Milestones & monitored risks", workspace_shell)
        self.assertIn("Background docs, decisions & search", workspace_shell)
        self.assertIn("Project details", workspace_shell)
        self.assertNotIn("Project details & danger zone", workspace_shell)
        self.assertNotIn("${renderStageTabs(activeKey)}", workspace_shell)
        self.assertIn("Quick links", workspace_shell)
        self.assertNotIn("Supporting tools", workspace_shell)
        self.assertIn('<section class="panel quick-links">', workspace_shell)
        self.assertNotIn('<details class="supporting-tools">', workspace_shell)
        self.assertIn("renderProjectSettings(ctx.project || {}, ctx.renderStageTabs)", settings_stage)
        self.assertGreater(settings_stage.index("Danger zone"), settings_stage.index("renderStageTabs('settings')"))
        self.assertIn(".quick-links .stage-tabs { display:grid; grid-template-columns:1fr", page)
        self.assertIn(".quick-links .stage-tab { width:100%; align-items:flex-start; white-space:normal;", page)
        self.assertIn(".quick-links .stage-tab .step-hint { display:block; overflow-wrap:anywhere; line-height:1.25;", page)
        self.assertIn("Monitored risks", setup_stage)
        self.assertNotIn("Memory search maintenance", setup_stage)
        self.assertIn("Save memory item", memory_stage)
        self.assertIn("Refresh memory search", memory_stage)
        self.assertIn("Memory health", memory_stage)
        self.assertIn("Fresh project note", memory_stage)
        self.assertIn("Ask works even before memory is indexed", memory_stage)
        self.assertIn("Queued for indexing", memory_stage)
        self.assertIn("config.queuedEndpoint ? 'Queued project update report generation...'", shared_js)
        self.assertIn("DRAFT_WORKFLOW_DATA_HANDLING_COPY", shared_js)
        self.assertIn("PROJECT_UPDATE_DATA_HANDLING_COPY", shared_js)
        self.assertIn("renderDraftWorkflowDataHandlingBanner('quiet', PROJECT_UPDATE_DATA_HANDLING_COPY)", shared_js)
        self.assertNotIn("Review before approving", shared_js)
        self.assertNotIn("<summary>Data handling</summary>", shared_js)
        self.assertNotIn("Notes/transcripts are sent to the server", shared_js)
        self.assertNotIn("Browser autosave stays on this device until you save", shared_js)
        self.assertNotIn("saved reports keep the source text and draft", shared_js)
        self.assertNotIn("<strong>Draft workspace:</strong>", shared_js)
        self.assertNotIn("Saving a report stores the transcript, draft report and project context", shared_js)
        self.assertIn("Draft workflow and data handling", shared_js)
        self.assertIn("uploaded or pasted transcripts are sent to the server to generate draft meeting minutes", shared_js)
        self.assertIn("Saved jobs retain both the transcript and generated minutes until the job is deleted", shared_js)
        ...
        self.assertIn('<section id="advancedDetailsPanel" class="support-footer hidden"', shared_js)
        self.assertIn('<details class="support-details support-details-compact raw-json">', shared_js)
        self.assertIn('Need audit/debug info?', shared_js)
        self.assertNotIn('<section id="advancedDetailsPanel" class="panel hidden support-panel">', shared_js)
        self.assertNotIn('Support / audit details', shared_js)
        self.assertNotIn('Advanced/support details', shared_js)
        self.assertNotIn('Raw output data', shared_js)
    ...
    def test_project_update_browsing_routes_are_registered(self):
        ...
        self.assertIn("params.set('stage', 'memory')", server)
        ...
        workspace_page = (REPO_DIR / "views" / "project-update-workspace.html").read_text(encoding="utf-8")
        workspace_shell = (REPO_DIR / "public" / "project-workspace.js").read_text(encoding="utf-8")
        ...
        self.assertIn("res.redirect(302, '/project-update-test?stage=settings')", server)
        ...
        self.assertNotIn("View your current project position, project profile and recent reports. Future reports are compared against this baseline. Edit milestones and monitored risks in Project settings.", insights_stage)
        self.assertNotIn("panel insights-header-panel", insights_stage)
        self.assertIn('id="ask-this-project"', insights_stage)
        self.assertIn("window.ProjectStages.ask", insights_stage)
        self.assertIn("ask-panel-standalone", insights_stage)
        self.assertNotIn("location.hash === '#ask-this-project'", insights_stage)
        ...
        self.assertIn("Save snapshot", insights_stage)
        self.assertIn("Approve baseline", insights_stage)
        self.assertIn("Tidy drafts", insights_stage)
        self.assertNotIn("More options", insights_stage)
        self.assertNotIn("compact-options", insights_stage)
        self.assertIn("section-title-row", workspace_page)
        self.assertNotIn("Review the current project position, recent report history and stored project memory", insights_stage)
        self.assertNotIn("Workspace management</summary>", insights_stage)
        self.assertNotIn("What these do", insights_stage)
        self.assertNotIn("Save profile snapshot", insights_stage)
        self.assertNotIn("Approve active baseline", insights_stage)
        self.assertIn("Milestone progress", insights_stage)
        self.assertIn("latest progress against the monitored milestones", insights_stage)
        self.assertNotIn("Latest milestone assessments", insights_stage)
        self.assertIn("Milestones", setup_stage)
        self.assertIn("Monitored risks", setup_stage)
        self.assertIn("standing risks you want each project update to keep checking", setup_stage)
        self.assertIn("structured project setup, not searchable memory items", setup_stage)
        self.assertIn("data-risk-id", setup_stage)
        self.assertIn("data-save-risk", setup_stage)
        self.assertIn("data-delete-risk", setup_stage)
        self.assertIn("Add monitored risk", setup_stage)
        self.assertIn("Monitored risk added.", setup_stage)
        ...
        self.assertIn("aria-label=\"Project summary\"", insights_stage)
        self.assertIn("Overall position", insights_stage)
        self.assertIn("monitored milestone", insights_stage)
        self.assertIn("monitored risk", insights_stage)
        self.assertIn("Latest snapshot", insights_stage)
        self.assertIn("Project health overview", insights_stage)
        ...
        self.assertLess(insights_stage.index("At a glance"), insights_stage.index("Project health overview"))
        self.assertLess(insights_stage.index("Project health overview"), insights_stage.index("Milestone progress"))
        self.assertLess(insights_stage.index("Project health overview"), insights_stage.index("Milestone progress"))
        self.assertLess(insights_stage.index("Milestone progress"), insights_stage.index("Recent reports"))
        self.assertIn("Project settings", settings_stage)
        self.assertNotIn("project-settings-panel", insights_stage)
        self.assertNotIn("Save project settings", insights_stage)
        self.assertIn("profile-editor-table", setup_stage)
        self.assertIn("profile-title-field", setup_stage)
        self.assertNotIn("data-expand-edit-row>Expand</button>", setup_stage)
        self.assertNotIn("Expanded row editor", setup_stage)
        self.assertIn("row-edit-modal-backdrop", workspace_page)
        self.assertIn("data-save-risk", setup_stage)
        self.assertIn("Could not remove monitored risk", setup_stage)
        self.assertIn("PW.request(`risks/", setup_stage)
        ...
        self.assertIn("data-expand-editor-row", reports_page)
        self.assertIn("data-expand-editor-row>Open row</button>", reports_page)
        self.assertNotIn("data-expand-editor-row>Expand</button>", reports_page)
        self.assertNotIn("Expanded row editor", reports_page)
        self.assertIn("openEditorRowModal", reports_page)
        self.assertIn("row-edit-modal-backdrop", reports_page)
        self.assertIn("Apply to row", reports_page)
        ...
        self.assertIn("Need audit/debug info?", reports_page)
        self.assertIn("support-details support-details-compact", reports_page)
        self.assertIn("Editable report JSON", reports_page)
        self.assertNotIn("Advanced/support: raw report data", reports_page)
        ...
        self.assertIn("select-trend", reports_page)
        self.assertIn("select-delivery", reports_page)
        self.assertIn("`Milestone ${index + 1} delivery status`, 'select-delivery')", reports_page)
        self.assertIn("`${titleLabel(key)} trend`, 'select-trend')", reports_page)
        self.assertNotIn("`Milestone ${index + 1} delivery status`, 'input')", reports_page)
        self.assertNotIn("`${titleLabel(key)} trend`, 'input')", reports_page)
        self.assertIn("Overall status", reports_page)
        self.assertIn("renderColourWithLabel", reports_page)
        self.assertIn("colour-with-label", reports_page)
        self.assertIn("statusBadge", reports_page)
        self.assertIn("status-badge", reports_page)
        self.assertIn("direct-edit-panel", reports_page)
        self.assertIn("The detailed milestone, risk and action rows are editable directly in their report tables below.", reports_page)
        self.assertIn("Edit these cells directly. Use Open row when a row has too much text for the table view.", reports_page)
        self.assertIn("resizeEditorTextarea", reports_page)
        self.assertIn("renderMilestoneEditorRows(milestones)", reports_page)
        self.assertNotIn("${milestones.map((item) => `<tr><td>${escapeHtml(titleLabel(item.milestone))}", reports_page)
        self.assertNotIn("<td>${statusBadge(item.health_assessment)}</td>", reports_page)
        self.assertIn("<td>${editableCell('relatedMilestone', action.related_milestone || action.relatedMilestone || '', `Action ${index + 1} milestone`)}</td>", reports_page)
        self.assertNotIn("<td>${editableCell('relatedMilestone', action.related_milestone || action.relatedMilestone || '', `Action ${index + 1} milestone`, 'input')}</td>", reports_page)
        self.assertIn("<td>${editableCell('milestone', item.milestone || '', `Milestone ${index + 1}`)}</td>", reports_page)
        self.assertIn("<td>${editableCell('riskTitle', risk.riskTitle || '', `Risk ${index + 1} title`)}</td>", reports_page)
        ...
        self.assertIn("Backend response", reports_page)
        self.assertIn("Version history", reports_page)
        self.assertIn("<th>Type</th><th>Change</th>", reports_page)
        self.assertIn("View version data", reports_page)
        self.assertNotIn("<h2>Versions</h2>", reports_page)
        self.assertNotIn("Original response", reports_page)
        ...
        self.assertIn("day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit'", reports_page)
        self.assertIn("function displayDate", reports_page)
        self.assertIn("function dateInputValue", reports_page)
        self.assertIn("tag === 'date'", reports_page)
        self.assertIn('type="date"', reports_page)
        ...
        self.assertIn("editableCell('baseline_finish_date', item.baseline_finish_date", reports_page)
        self.assertIn("editableCell('deadline', action.deadline", reports_page)
        self.assertIn("`Milestone ${index + 1} baseline`, 'date')", reports_page)
        self.assertIn("`Milestone ${index + 1} forecast`, 'date')", reports_page)
        self.assertIn("`Action ${index + 1} deadline`, 'date')", reports_page)
        self.assertNotIn("`Milestone ${index + 1} baseline`, 'input')", reports_page)
        self.assertNotIn("`Milestone ${index + 1} forecast`, 'input')", reports_page)
        self.assertNotIn("`Action ${index + 1} deadline`, 'input')", reports_page)
        ...
        self.assertIn("Baseline</th>", setup_stage)
        self.assertIn("Included", memory_stage)
        self.assertIn("Edit memory item", memory_stage)
        self.assertIn("data-edit-knowledge", memory_stage)
        self.assertIn("Cancel edit", memory_stage)
        self.assertIn("method: editingItemId ? 'PATCH' : 'POST'", memory_stage)
        self.assertIn("Continue latest project", workspace_shell)
        self.assertIn("quickActionButton('ask', 'Ask project'", workspace_shell)
        self.assertIn("quickActionButton('memory', 'Add memory'", workspace_shell)
        self.assertIn("quickActionButton('reports', 'View reports'", workspace_shell)
        self.assertIn("requestedProjectId", workspace_shell)
        self.assertNotIn("<th>Official</th>", setup_stage)
        self.assertNotIn("standing context such as", setup_stage)
