...
class ProjectUpdateMiniLMWorkflowTest(unittest.TestCase):
    ...
    def test_project_update_page_enables_editable_report_ui(self):
        ...
        self.assertIn("Review before approving", shared_js)
        self.assertIn("<summary>Data handling</summary>", shared_js)
        self.assertIn("Notes/transcripts are sent to the server", shared_js)
        self.assertIn("Browser autosave stays on this device until you save", shared_js)
        self.assertIn("saved reports keep the source text and draft", shared_js)
