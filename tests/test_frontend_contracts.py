...
class FrontendContractTest(unittest.TestCase):
    ...
    def test_meeting_minutes_agent_uses_evidence_backed_resumable_workflow(self):
        ...
        self.assertIn("router.get('/meeting-minutes-agent/drafts', requireAuth", api)
        self.assertIn("router.patch('/meeting-minutes-agent/drafts/:draftId', requireAuth", api)
        self.assertIn("router.patch('/meeting-minutes-agent/drafts/:draftId/source/:unitId', requireAuth", api)
        self.assertIn("router.post('/meeting-minutes-agent/drafts/:draftId/audit-actions', requireAuth", api)
        self.assertIn("router.post('/meeting-minutes-agent/drafts/:draftId/proposal', requireAuth", api)
        self.assertIn("router.post('/meeting-minutes-agent/drafts/:draftId/undo', requireAuth", api)
        self.assertIn("router.post('/meeting-minutes-agent/drafts/:draftId/export.docx', requireAuth", api)
        self.assertIn("router.post('/meeting-minutes-agent/drafts/:draftId/export.pdf', requireAuth", api)
        self.assertIn("process.env.POWER_AUTOMATE_AGENT_WEBHOOK_URL", api)
        self.assertIn("JSON.stringify({ input: { prompt } })", api)
        self.assertIn("askPowerAutomateMeetingMinutesAgent(prompt)", api)
        ...
        self.assertIn('<body class="meeting-agent-page">', page)
        ...
        self.assertIn('id="meetingDate" type="date"', page)
        self.assertIn('id="internalAttendees"', page)
        self.assertIn('id="clientAttendees"', page)
        self.assertIn('id="clientAttendeeLabelSelect"', page)
        self.assertIn('value="External"', page)
        self.assertNotIn('id="attendees"', page)
        self.assertIn("/api/meeting-minutes-agent/prepare", client)
        self.assertIn("/api/meeting-minutes-agent/generate", client)
        self.assertIn("draftId:state.draft.draftId", client)
        self.assertIn("revision:state.draft.revision", client)
        self.assertIn("agentRetryDelaysSeconds = [5, 15, 30]", client)
        self.assertIn("function waitForAgentRetry", client)
        self.assertIn("Retrying the agent now", client)
        self.assertIn("error.code === 'M365_AGENT_USAGE_LIMIT' && error.retryable", client)
        self.assertIn("code: 'M365_AGENT_USAGE_LIMIT'", api)
        self.assertIn("retryable: true", api)
        ...
        self.assertIn('data-owner-choice', client)
        self.assertIn('Another person', client)
        self.assertIn('data-timing-date data-action-index=', client)
        self.assertIn("function participantNames", client)
        self.assertIn("data-move-attendee", client)
        self.assertIn('class="delete attendee-remove attendee-remove-icon"', client)
        self.assertIn('data-lucide="trash-2"', client)
        self.assertIn('class="visually-hidden"', client)
        self.assertNotIn("data-remove-attendee type=\"button\" aria-label=\"Remove ' + escapeHtml(name || 'attendee') + '\">Remove</button>", client)
        self.assertIn('.attendee-chip input{font-size:.9rem', page)
        self.assertIn('.attendee-chip .attendee-move{font-size:.82rem', page)
        self.assertIn("clientAttendeeLabel", client)
        self.assertIn("overflow:hidden;min-height:52px", page)
        self.assertIn('#workflowStatus[hidden]{display:none!important}', page)
        self.assertIn('@keyframes agent-status-spin', page)
        self.assertIn('aria-busy="false"', page)
        self.assertIn("status.setAttribute('aria-busy', busy ? 'true' : 'false')", client)
        self.assertIn('<details id="reviewFlags"', page)
        self.assertNotIn('<details id="reviewFlags" class="panel review-flags" open', page)
        self.assertIn("uncertain_fact:'Uncertain detail'", client)
        self.assertIn("if (flags.length && wasHidden) panel.open = false", client)
        self.assertIn("add:'New item'", client)
        self.assertIn("modify:'Suggested edit'", client)
        ...
        self.assertEqual(page.count('class="agent-edit-toggle-closed">Open editor'), 2)
        self.assertEqual(page.count('class="agent-edit-toggle-open">Close editor'), 2)
        self.assertEqual(page.count('data-lucide="chevron-down"'), 2)
        self.assertIn('.agent-edit[open] .agent-edit-toggle svg', page)
        self.assertIn('.topic-field input', page)
        self.assertIn('font-weight:700', page.split('.topic-field input', 1)[1].split('}', 1)[0])
        self.assertIn('<span class="topic-label">Topic</span>', client)
        self.assertIn("points: 'Discussion point'", client)
        self.assertIn("decisions: 'Decision'", client)
        self.assertIn("openQuestions: 'Open question'", client)
        self.assertIn('class="record-position"', client)
        self.assertIn('.record-row:nth-child(even)', page)
        self.assertIn('.record-row:focus-within', page)
        self.assertIn('.actions-table tbody tr:nth-child(even)', page)
        ...
        self.assertNotIn("function restoreUnit", client)
        self.assertNotIn("data-restore-unit", client)
        self.assertIn("Include evidence appendix", page)
        self.assertIn("Download Word", page)
        self.assertIn('src="https://unpkg.com/lucide@latest/dist/umd/lucide.min.js"', page)
        self.assertIn('data-lucide="arrow-right"', page)
        self.assertIn('id="saveMinutes" class="button button-with-icon"', page)
        self.assertIn('data-lucide="save"', page)
        self.assertIn('id="printMinutes" class="secondary button-with-icon"', page)
        self.assertIn('data-lucide="printer"', page)
        self.assertEqual(page.count('data-lucide="download"'), 2)
        self.assertIn('id="downloadPdf" class="button button-with-icon"', page)
        self.assertIn('id="downloadWord" class="button button-with-icon"', page)
        self.assertIn("window.lucide.createIcons()", client)
        ...
        self.assertIn("/export.pdf", client)
        self.assertIn("Generating your PDF", client)
    ...
    def test_meeting_minutes_final_queue_and_jobs_page_are_wired(self):
        ...
        self.assertNotIn('id="overallTopics"', staged_page)
        ...
        self.assertNotIn('id="meetingPurpose"', staged_page)
        self.assertNotIn('id="keyFactsList"', staged_page)
    ...
    def test_resolved_workstream_flags_do_not_silently_block_actions_transition(self):
        ...
        self.assertIn("if (activeIndex === 1 && reconcileUnresolvedWorkstreamFlags().length)", staged_page)
        self.assertNotIn("if (activeIndex === 1 && Object.keys(unresolvedWorkstreamFlags).length)", staged_page)
    ...
    def test_staged_review_items_are_connected_to_transition_and_autosave_is_visible(self):
        ...
        self.assertGreaterEqual(staged_page.count('class="stage-autosave-status"'), 4)
