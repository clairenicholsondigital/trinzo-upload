...
class FrontendContractTest(unittest.TestCase):
    ...
    def test_meeting_minutes_agent_uses_evidence_backed_resumable_workflow(self):
        ...
        agent_v2 = (REPO_DIR / "utils" / "meetingMinutesAgentV2.js").read_text(encoding="utf-8")
        ...
        self.assertIn('class="delete icon-only"', client)
        self.assertIn("icon('trash')", client)
        ...
        self.assertIn('.attendee-chip input{font-size:var(--fs-sm)', page)
        ...
        self.assertIn('Review suggested changes', page)
        ...
        self.assertIn('normaliseAgentDeclaredProposals', api)
        self.assertIn('grid-template-columns:auto minmax(0,1fr)', page)
        self.assertIn('function updateProposalSelection()', client)
        ...
        self.assertEqual(page.count('class="agent-edit agent-edit-inline"'), 2)
        self.assertEqual(page.count('>Ask AI to edit</summary>'), 2)
        ...
        self.assertIn('.editable-actions-table th:nth-child(1){width:60%}', page)
        ...
        self.assertIn('Related facts are grouped here', client)
        self.assertIn('Promote to minutes', client)
        ...
        self.assertIn('id="downloadWord"', page)
        ...
        self.assertIn('Save final minutes', page)
        self.assertIn('id="mobileStepSelect"', page)
        self.assertIn('.meeting-agent-page.has-draft .hero{display:none}', page)
        self.assertIn('Check transcript for more actions', page)
        self.assertIn("'Regenerate summary' : 'Create summary'", client)
        self.assertIn("+ ' to check' : 'Checks complete'", client)
        self.assertIn("function icon(name)", client)
        ...
        self.assertIn("actionEditorState = { pendingRows: {}, customOwners: {}, editingId: '' }", client)
        ...
        # A reviewer's own edits are flagged against the evidence, never stripped
        # back out from under them; only agent output is enforced.
        self.assertIn("{ enforceEvidence: false }", api)
        self.assertIn("const enforceEvidence = options.enforceEvidence !== false", agent_v2)
        # A save conflict keeps what is on screen and offers an explicit reload.
        self.assertIn('id="reloadDraft"', page)
        self.assertIn("your edits are here but unsaved", client)
        self.assertNotIn("if (error.currentDraft) adoptDraft(error.currentDraft)", client)
